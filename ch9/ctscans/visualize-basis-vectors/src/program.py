...
parser = argparse.ArgumentParser(description='Convert DRMs into DICOMs and Images')
...
def download_folder(bucket_name = 'your-bucket-name', bucket_dir = 'your-bucket-directory/', dl_dir= "local-dir/"):
    storage_client = storage.Client()
...
for p in range(len(percs)):
    perc = percs[p]
    diags = [diags_orig[i] if i < round(len(diags) - (len(diags) * perc)) else 0 for i in range(len(diags))]
    recon = drmU_p5 @ np.diag(diags) @ drmV_p5.transpose()
    # plot_3d_matrix(recon.transpose().reshape((512,512,301)), (512,512,301), 1.0, 0.810547, 1.2337347494963278)
    composite_img = recon.transpose().reshape((512,512,301))
